...
DECLARE
  new_user_id uuid;
BEGIN
  -- Seed user Guilherme Almeida (idempotent: skip if email already exists)
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'willhelmalmeida@gmail.com') THEN
    ...
    INSERT INTO auth.users (
      ...
      crypt('DpsJur@2026!Colab', gen_salt('bf')),
      NOW(), NOW(), NOW(),
      '{"provider": "email", "providers": ["email"]}',
      '{"name": "Guilherme Almeida", "role": "User", "color": "#3b82f6", "canViewFinance": false}',
    ...
    -- Ensure public profile is created with exact colaborador (User) settings
    INSERT INTO public.profiles (id, email, name, role, "canViewFinance", color)
    VALUES (new_user_id, 'willhelmalmeida@gmail.com', 'Guilherme Almeida', 'User', false, '#3b82f6')
